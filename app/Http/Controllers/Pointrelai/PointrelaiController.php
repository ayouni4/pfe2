...
class PointrelaiController extends Controller
{
    public function form_register(Request $request){
        return view  ('pointrelai.register');

    }
    public function form_login(Request $request){
       
        return view  ('pointrelai.login');
    ...
    public function traitement_login(Request $request){
        ...
        if($user){
            if(Hash::check($request->input('password'),$user->password)){
                ...
                return  redirect('/pointrelai/formulaire');//commande
            }else{
