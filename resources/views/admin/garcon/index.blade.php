...
<div class="mt-3">
<div id="tableExample2" data-list='{"valueNames":["nom","prenom","adresse","typetransport","numero","actions"],"page":5,"pagination":true}'>
    ...
    <table class="table table-bordered table-striped fs--1 mb-0">
      <thead class="bg-200 text-900">
        ...
        <th class="sort" data-sort="name">#</th>
          ...
          <th class="sort" data-sort="typetransport">typetransport</th>
      ...
      @foreach($garcons as $index =>$c)
        <tr>
          ...
          <td>{{ $c->nom }}</td>
           ...
           <td>{{ $c->numero }}</td>
          <td>{{ $c->adresse }}</td>
          <td>{{ $c->typetransport }}</td>
        <td>{{ $c->matricule }}</td>
        <td>
         <a  data-bs-toggle="modal" data-bs-target="#editGarcon{{ $c->id}}" ><span class="badge fs--1 bg-secondary">modifier</span></a>
         <a onclick="return confirm('voulez-vous vraiment supprimer?') " href="/admin/garcon/{{ $c->id}}/delete" ><span class="badge fs--1 bg-danger">supprimer</span></a>
        </td>
    </tr>
    @endforeach
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Ajouter garcon</h5><button class="btn p-1" type="button" data-bs-dismiss="modal" aria-label="Close"><span class="fas fa-times fs--1"></span></button>
      </div>
      <form action="/garcon/formulaire/traitement" method="post">
        @csrf
...
<select class="form-select" aria-label="Default select example"  name="typetransport" placeholder="typetransport" required>
  <option selected="bicyclette">bicyclette</option>
...
@foreach($garcons as $index =>$c)
<!-- modal modifier-->
<div class="modal fade" id="editGarcon{{ $c->id}}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">modifier garcon : <span class="text-primary">{{ $c->nom }}</span></h5><button class="btn p-1" type="button" data-bs-dismiss="modal" aria-label="Close"><span class="fas fa-times fs--1"></span></button>
      </div>
      <form action="/admin/garcon/{id}/update" method="post">
        @csrf
      <div class="modal-body">
...
<select class="form-select" aria-label="Default select example"  name="typetransport"  value="{{$c->typetransport}} placeholder="typetransport" required>
  <option selected="bicyclette">bicyclette</option>
  <option value="moto">moto</option>
                  ...
                  <input type="hidden" value="{{ $c->id}}" name="id_garcon">
      </div>
      <div class="modal-footer">
        <button class="btn btn-primary" type="submit">Okay</button>
        <p class="alert alert-danger">{{session('status')}}</p>
